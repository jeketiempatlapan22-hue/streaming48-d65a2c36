...
const Admin = () => {
  ...
  const resetForm = () => {
    setTitle(""); setUrl(""); setType("m3u8"); setIsActive(true); setEditingId(null); setShowForm(false);
  };

  const handleEdit = (stream: Stream) => {
    setTitle(stream.title); setUrl(stream.url); setType(stream.type as StreamType);
    setIsActive(stream.is_active); setEditingId(stream.id); setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await supabase.from("streams").update({ title: title.trim(), url: url.trim(), type, is_active: isActive }).eq("id", editingId);
      if (error) toast.error("Gagal mengupdate stream");
      else toast.success("Stream berhasil diupdate");
    } else {
      const { error } = await supabase.from("streams").insert({ title: title.trim(), url: url.trim(), type, is_active: isActive, created_by: user?.id });
      if (error) toast.error("Gagal menambahkan stream");
      else toast.success("Stream berhasil ditambahkan");
    }

    resetForm(); setSubmitting(false); fetchStreams();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    else { toast.success("Stream dihapus"); fetchStreams(); }
  };

  const handleToggleActive = async (stream: Stream) => {
    const { error } = await supabase.from("streams").update({ is_active: !stream.is_active }).eq("id", stream.id);
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border">
        <div className="container max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href="/" className="px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">Beranda</a>
            <button
      ...
      <main className="container max-w-5xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          ...
          >
            <Plus className="w-4 h-4" /> Tambah Stream
          </button>
        </div>

        {showForm && (
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4 shadow-xl shadow-black/20 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Nama stream"
                    className="w-full bg-muted border border-border rounded-lg px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/50" required />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="URL stream atau video ID"
                  className="w-full bg-muted border border-border rounded-lg px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/50" required />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="is_active" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} className="rounded border-border" />
        ...
        {loadingStreams ? (
          ...
          <div className="space-y-3">
            {streams.map((stream) => (
              <div key={stream.id}
                className="bg-card border border-border rounded-2xl p-4 flex items-center gap-4 hover:border-primary/30 transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium uppercase tracking-wider ${stream.is_active ? "bg-[hsl(var(--success))]/10 text-[hsl(var(--success))]" : "bg-muted text-muted-foreground"}`}>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleToggleActive(stream)}
