...
BEGIN
  ...
  -- NEW: For membership shows, also create an anonymous confirmed entry in subscription_orders
  -- so admin can see and manage it from the Subscription Order Manager. Phone/email left NULL
  -- for admin to fill in later.
  IF COALESCE(_show.is_subscription, false) = true THEN
    _sub_short_id := 'RB-' || upper(substring(replace(gen_random_uuid()::text, '-', '') from 1 for 6));
    INSERT INTO public.subscription_orders (
      show_id, user_id, phone, email, status, payment_status,
      payment_method, short_id, created_at
    ) VALUES (
      _show.id, NULL, NULL, NULL, 'confirmed', 'confirmed',
      'reseller_bot', _sub_short_id, now()
    );
  END IF;
