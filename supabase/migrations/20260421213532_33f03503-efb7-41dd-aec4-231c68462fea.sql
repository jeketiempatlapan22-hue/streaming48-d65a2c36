-- Update reseller_create_token & reseller_create_token_by_id agar expires_at
-- mengikuti jadwal show: jika jadwal di masa depan, expires = schedule + duration_days.
-- Jika tidak ada jadwal valid atau sudah lewat, fallback ke now() + duration_days.
...
BEGIN
  ...
  -- Anchor expiry to the show schedule when it is in the future,
  -- so a 1-day token for a show scheduled tomorrow stays valid tomorrow.
