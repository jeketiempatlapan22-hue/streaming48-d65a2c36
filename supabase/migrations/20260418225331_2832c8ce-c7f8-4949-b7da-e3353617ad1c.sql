-- Lock self_reset_token_session: hanya untuk token single-device.
-- Token multi-device (max_devices > 1) harus reset oleh admin.
...
BEGIN
  ...
  -- Multi-device tokens TIDAK boleh self-reset
  IF COALESCE(t.max_devices, 1) > 1 THEN
